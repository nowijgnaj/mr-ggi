...
y.pair = FineMapping(x.double,y) #cis SNP 찾기
...
mic <- MGI(y.pair, y, calc.idx = result2, c.size = 5)
# 어떤건 x가 앞이고 어떤건 x가 뒤다.. 뭐냐?
head(mic$edges); head(mic$nodes)

mic2 <- MGI(y.pair, y, calc.idx = result2, c.size = 5, GRN = TRUE)
